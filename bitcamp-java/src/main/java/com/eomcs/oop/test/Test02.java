...
public class Test02 {
  public static void main(String[] args) {
    ...
    // 이렇게 개발자가 임의로 만든 데이터 타입을 "사용자 정의 데이터 타입"
    // "사용자 정의 데이터 타입(user defined data type)"이라 부른다.
    // 개발자가 class라는 문법으로 정의한 데이터 타입으로 메모리를 만들 때는 일반적인 방식으로 만들 수 없다.
    // new 명령을 사용해야 한다.
    // 준비된 메모리를 사용하기 위해서는 그 메모리의 주소를 변수에 보관해야 한다.
    // 이렇게 class 설계도로 만든 메모리의 주소를 저장하는 변수를 "레퍼런스(reference)"라고 한다.
    ...
    // - 이 명령을 통해 생성된(사용할 수 있도록 준비된) 메모리를 "인스턴스(instance)" 라고 한다. 보통은 "객체(object)"라고 부른다.
